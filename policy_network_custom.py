...
class PolicyNetwork:
    def __init__(self, input_dim=0, output_dim=0, lr=0.01):
        ...
        self.model.add(LSTM(256, input_shape=(1, input_dim),
                            return_sequences=True, stateful=False, dropout=0.3))
        self.model.add(BatchNormalization())
        self.model.add(LSTM(128, return_sequences=True, stateful=False, dropout=0.3))
        self.model.add(BatchNormalization())
        self.model.add(LSTM(64, return_sequences=False, stateful=False, dropout=0.5))
        self.model.add(BatchNormalization())
        self.model.add(Dense(output_dim))
        self.model.add(Activation('sigmoid'))       # 숫자 바꿨음
