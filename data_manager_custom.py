"""
학습 데이터의 특징으로 주가와 거래량 관련 값을 고려했었음.
거기에 '기관순매수 거래량'과 '외국인순매수 거래량'을 추가
"""
import pandas as pd
import numpy as np

"""
기관 순매수를 inst, 외국인 순매수를 frgn으로 명명하고 추가함.
def load_chart_data(fpath):
    ...
    chart_data['frgn'] = pd.to_numeric(chart_data['frgn'].str.replace(',', ''), errors='coerce') 빈 문자열을 nan값으로
    return chart_data
"""
"""
'기관 순매수 거래량'과 '외국인 순매수 거래량'의 이동평균 값을 계산하는 코드 두 줄 추가
...
"""
새로 추가됨 (기관과 외국인 순매수 거래량의 전일 대비 비율)
