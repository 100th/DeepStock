...
""" 정책 신경망 출력 값을 기준으로 행동을 결정하는 로직을 커스터마이징
    정책 신경망은 매수와 매도에 대해 각각 출력 값을 주는데, DeepStock은 그 두 값 중에서
    높은 값을 선택하여 투자 행동 결정을 내린다.
    이 경우, 정책 신경망 출력 값이 매우 낮았는데도 매수 또는 매도를 수행할 수 있다.
    정책 신경망의 출력 값이 어느 정도의 값(임계값, threshold) 이상일 경우에만 결정된 행동을 결정하도록 할 수 있음
    여기서는 정책 신경망의 출력 값이 0.1 이상일 경우에만 행동을 수행하고 0.1 미만일 경우 관망할 수 있음
        else:
            exploration = False
